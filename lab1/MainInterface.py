...
def create_gui(root):
    ...
    #style.configure("TButton", background="#000000", foreground="#000000", padding=10, borderwidth=0, relief="flat")
    style.configure("TButton", background="#e0e0e0", foreground="#000000", padding=10, borderwidth=0, relief="flat")
    #style.configure("TEntry", fieldbackground="#fff", foreground="#000", padding=5, relief="flat")
    style.configure("TEntry", fieldbackground="#f0f0f0", foreground="#000", padding=5, relief="flat")
    #style.map("TEntry", fieldbackground=[('readonly', '#ddd')])  # Выделение нередактируемых полей
    ...
    min_length_var = tk.IntVar(value=1)  # Используем IntVar для целых чисел
    min_length_scale = ttk.Scale(length_frame, from_=1, to=20, orient="horizontal",command=lambda value: min_length_var.set(int(value)),variable=min_length_var)
    ...
    max_length_var = tk.IntVar(value=10)  # Используем IntVar для целых чисел
    max_length_scale = ttk.Scale(length_frame, from_=1, to=20, orient="horizontal",command=lambda value: max_length_var.set(trunc(int(value))),variable=max_length_var)
    ...
    generate_length_button = ttk.Button(length_frame, text="Сгенерировать",command=lambda: generate_all_sequences(grammar_file_var.get(),min_length_var.get(),max_length_var.get(),result_text))
    generate_length_button.grid(row=1, column=0, columnspan=4, pady=5)

    #generate_length_button = ttk.Button(length_frame, text="Сгенерировать", command=lambda: generate_all_sequences(grammar_file_var,min_length_entry, max_length_entry,result_text))
    #generate_length_button.grid(row=1, column=2, columnspan=4, pady=5)
    ...
    generate_button = ttk.Button(choice_frame, text="Изображение дерева", command=lambda: generate_and_display_tree(grammar_file_var, min_length_var, max_length_var,choise_entry,result_text))
